...
def evaluate(args, loader, generator, num_samples):
    with torch.no_grad():
        for batch in loader:
            ...
            # modified by zyl 2020/12/19 (obs_traj, pred_traj_gt, obs_traj_rel, pred_traj_gt_rel, non_linear_ped, loss_mask, seq_start_end) = batch
            (obs_traj, pred_traj_gt, obs_traj_rel, pred_traj_gt_rel, obs_traj_rel_v, pred_traj_rel_v, obs_traj_g, pred_traj_g,
             non_linear_ped, loss_mask, seq_start_end) = batch

            for _ in range(1):  # num_samples
                # modified by zyl 2020/12/19 pred_traj_fake_rel = generator(obs_traj, obs_traj_rel, seq_start_end)
                pred_traj_fake_rel = generator(obs_traj, obs_traj_rel, seq_start_end, obs_traj_g)
                pred_traj_fake = relative_to_abs(pred_traj_fake_rel, obs_traj[-1])
                gt = (pred_traj_gt[:, 3, :].data).cpu()
                input_a = (obs_traj[:, 3, :].data).cpu()
                out_a = (pred_traj_fake[:, 3, :].data).cpu()
